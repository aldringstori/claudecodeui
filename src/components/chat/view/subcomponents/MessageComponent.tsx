    ...
    >
      {message.type === 'user' ? (
        ...
        <div className="flex items-end space-x-0 sm:space-x-3 w-full sm:w-auto sm:max-w-[85%] md:max-w-md lg:max-w-lg xl:max-w-xl">
          <div className="bg-primary text-primary-foreground rounded-2xl rounded-br-md px-3 sm:px-4 py-2.5 shadow-md flex-1 sm:flex-initial group">
            <div className="text-sm font-medium whitespace-pre-wrap break-words leading-relaxed">
              {message.content}
            </div>
            ...
            <div className="flex items-center justify-end gap-1.5 mt-1.5 text-[10px] text-primary-foreground/70 uppercase tracking-widest font-bold">
              <button
                type="button"
          ...
          {!isGrouped && (
            <div className="hidden sm:flex w-9 h-9 bg-primary/20 border border-primary/30 rounded-xl items-center justify-center text-primary font-bold text-xs flex-shrink-0">
              U
            </div>
          )}
        </div>
        ...
        <div className="w-full">
          <div className="flex items-center gap-2 py-0.5">
            <span className={`inline-block w-1.5 h-1.5 rounded-full flex-shrink-0 ${message.taskStatus === 'completed' ? 'bg-status-success' : 'bg-status-warning'}`} />
        ...
        <div className="w-full">
          {!isGrouped && (
            <div className="flex items-center space-x-3 mb-2.5">
              {message.type === 'error' ? (
                <div className="w-9 h-9 bg-destructive/10 border border-destructive/20 rounded-xl flex items-center justify-center text-destructive text-sm flex-shrink-0">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                  </svg>
                </div>
              ) : message.type === 'tool' ? (
                <div className="w-9 h-9 bg-muted/50 border border-border/50 rounded-xl flex items-center justify-center text-muted-foreground text-sm flex-shrink-0">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="w-9 h-9 rounded-xl flex items-center justify-center text-white text-sm flex-shrink-0 p-1.5 bg-muted/30 border border-border/40">
                  <SessionProviderLogo provider={provider} className="w-full h-full opacity-90" />
                </div>
              )}
              <div className="text-xs font-bold text-foreground tracking-widest uppercase opacity-80">
